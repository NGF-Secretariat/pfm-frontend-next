...
const initialValues = ["expenditure_by_sector_percentage_total_revenue"];
const ExpendituresBySectorTotalRevenue = ({ data, categories = [] }) => {
  ...
  React.useEffect(() => {
    const data = getAllCategories(BUDGET_CATEGORIES__PI[12].children);
    setOptions((prev) => [...prev, ...data]);
    //eslint-disable-next-line
  }, []);

  const name = "expenditure_by_sector_percentage_total_revenue";
  ...
  return (
    <React.Fragment>
      ...
      <AppTableCell
        titleClass=""
        title="Administration Sector Total Expenditure (% of Total Revenue)"
        values={data.map((item) => item[name]?.admin_sector_total_expenditure)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Economic Sector Total Expenditure (% of Total Revenue)"
        values={data.map(
      ...
      <AppTableCell
        titleClass=""
        title="Law and Justice Sector Total Expenditure (% of Total Revenue)"
        values={data.map(
      ...
      <AppTableCell
        titleClass=""
        title="Regional Sector Total Expenditure (% of Total Revenue)"
        values={data.map(
      ...
      <AppTableCell
        titleClass=""
        title="Social Sector Total Expenditure (% of Total Revenue)"
        values={data.map(
...
export default ExpendituresBySectorTotalRevenue;
