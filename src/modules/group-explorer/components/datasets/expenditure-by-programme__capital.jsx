...
const initialValues = ["exp_by_programme_capital"];
const ExpenditureByProgrammeCapital = ({ data, categories = [] }) => {
  ...
  React.useEffect(() => {
    const data = getAllCategories(BUDGET_CATEGORIES[8].children);
    setOptions((prev) => [...prev, ...data]);
    //eslint-disable-next-line
  }, []);
  const name = "exp_by_programme_capital";
    ...
    <React.Fragment>
      <tr className="sticky-top__sub bg-light">
        ...
        <td className="sticky-left fw-bold">
          Expenditure by Programme (Capital)
        </td>
        {data?.map((_, index) => (
...
export default ExpenditureByProgrammeCapital;
