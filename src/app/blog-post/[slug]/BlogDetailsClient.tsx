...
import { CalendarDays, User, ArrowLeft, Loader2, Edit, Trash2, AlertTriangle } from "lucide-react";
...
const formatHtmlWithLinks = (html: string): string => {
    if (!html) return "";
    
    return html.replace(/<a\s+(?:[^>]*?\s+)?href=["']([^"']+)["']/gi, (match, href) => {
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-wider mb-4">
                        <div className="flex items-center gap-1.5 bg-[#eafbf5] text-[#016630] px-3.5 py-1.5 rounded-full border border-[#bbf7df]">
                            <User size={14} className="text-[#1D9E75]" />
                            <span>Author: {blog.author || "NGF Secretariat"}</span>
                        </div>
                        <div className="flex items-center gap-1.5 bg-gray-100 text-gray-700 px-3.5 py-1.5 rounded-full border border-gray-200">
                            <CalendarDays size={14} className="text-[#1D9E75]" />
                            <span>Date Produced: {blog.date}</span>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-start">
                    ...
                    <div className="md:col-span-9 min-w-0 overflow-hidden break-words max-w-full">
                        <article
                            className="
                                prose prose-md sm:prose-lg max-w-none break-words
                                prose-headings:text-[#08542b] prose-headings:font-bold
                                prose-p:text-gray-700 prose-p:leading-8 prose-p:break-words
                                prose-a:text-[#016630] prose-a:font-semibold prose-a:underline prose-a:underline-offset-4 hover:prose-a:text-[#014c24] prose-a:break-all
                                prose-li:text-gray-700
                            "
                        >
                            {blog.content && (blog.content.trim().startsWith("<") || blog.content.includes("ql-align") || blog.content.includes("<p")) ? (
                                <div
                                    className="blog-rich-content text-gray-700 leading-8 break-words max-w-full [&_img]:rounded-2xl [&_img]:shadow-md [&_img]:mx-auto [&_img]:my-6 [&_img]:max-h-[550px] [&_img]:w-full [&_img]:sm:w-auto [&_img]:object-contain [&_img]:border [&_img]:border-gray-100 [&_img]:block"
                                    dangerouslySetInnerHTML={{ __html: formatHtmlWithLinks(blog.content) }}
                                />
                            ) : (
                                <ReactMarkdown
                                    ...
                                    components={{
                                        a: ({ node, ...props }) => (
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-[#016630] font-semibold underline underline-offset-4 hover:text-[#014c24] transition-colors break-all cursor-pointer"
                                            />
                                        ),
                                        img: ({ node, ...props }) => (
                                            <img
                                                {...props}
                                                className="rounded-2xl shadow-md mx-auto max-h-[550px] w-full sm:w-auto object-contain my-6 border border-gray-100 block max-w-full"
                                                onError={(e: any) => {
