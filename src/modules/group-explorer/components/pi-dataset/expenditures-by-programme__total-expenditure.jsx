...
const initialValues = ["expenditure_by_programme_percentage_total_expenditure"];
const ExpenditureByProgrammeTotalExpenditure = ({ data, categories = [] }) => {
  ...
  React.useEffect(() => {
    const data = getAllCategories(BUDGET_CATEGORIES__PI[15].children);
    setOptions((prev) => [...prev, ...data]);
    //eslint-disable-next-line
  }, []);

  const name = "expenditure_by_programme_percentage_total_expenditure";
    ...
    <React.Fragment>
      <tr>
        <td className="sticky-left text-underline fw-semibold">
          Expenditure by Programme (% of Total Expenditure)
        </td>
        {data?.map((_, index) => (
      ...
      <AppTableCell
        titleClass=""
        title="Information Communication and Technology (% of Total Expenditure)"
        values={data.map(
      ...
      <AppTableCell
        titleClass=""
        title="Growing the Private Sector (% of Total Expenditure)"
        values={data.map((item) => item[name]?.growing_private_sector)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Reform of Government and Governance (% of Total Expenditure)"
        values={data.map(
      ...
      <AppTableCell
        titleClass=""
        title="Power (% of Total Expenditure)"
        values={data.map((item) => item[name]?.power)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Rail (% of Total Expenditure)"
        values={data.map((item) => item[name]?.rail)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Water Ways (% of Total Expenditure)"
        values={data.map((item) => item[name]?.water_ways)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Road (% of Total Expenditure)"
        values={data.map((item) => item[name]?.road)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Airways (% of Total Expenditure)"
        values={data.map((item) => item[name]?.airways)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="COVID-19 (% of Total Expenditure)"
        values={data.map((item) => item[name]?.covid_19)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="CLIMATE CHANGE (% of Total Expenditure)"
        values={data.map((item) => item[name]?.climate_change)}
        isVisible={filterCategories.includes(name)}
      />
      <AppTableCell
        titleClass=""
        title="Oil and Gas Infrastructure (% of Total Expenditure)"
...
export default ExpenditureByProgrammeTotalExpenditure;
